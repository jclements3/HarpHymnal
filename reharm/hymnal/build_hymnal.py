...
Mirrors ../retab/hymnal/build_hymnal.py but emits parallel level tracks so
the tablet app can flip through L1 / L2 / L3 for the same hymn. Writes:

  - /tmp/reharm_hymnal_L<level>/<slug>.abc    (intermediate ABC)
  - /tmp/reharm_hymnal_L<level>/<slug>.svg    (compiled by abcm2ps -g)
  - tablet_app/assets/reharm/hymns/L<level>/<slug>.svg   (viewBox-patched)
  - tablet_app/assets/reharm/reharm_hymns.js             (window.REHARM_HYMNS)

Run:

    python build_hymnal.py                  # builds L1, L2, L3 (default)
    python build_hymnal.py --levels 1,2,3,4 # custom level set
...
from reharm_hymnal import REHARM_LEVELS, build_abc  # noqa
...
DEST_ROOT = Path("/home/james.clements/projects/HarpHymnal/tablet_app/"
                 "app/src/main/assets/reharm/hymns")
INDEX_JS = Path("/home/james.clements/projects/HarpHymnal/tablet_app/"
                "app/src/main/assets/reharm/reharm_hymns.js")
...
def build_one_level(jpath: Path, level: int, stage: Path, dest: Path,
    ...
    """Build one hymn at one level. Returns (ok, n_svg_pages)."""
    slug = jpath.stem
    hymn = json.loads(jpath.read_text(encoding="utf-8"))

    reharmed = REHARM_LEVELS[level](hymn)
    abc = build_abc(reharmed, x_num=1, num_prefix=num_prefix)
...
def build_one_all_levels(jpath: Path, levels: list[int], num_prefix: str,
    ...
    for lvl in levels:
        try:
            ok, n_pages = build_one_level(jpath, lvl, stages[lvl], dests[lvl],
                                          num_prefix)
...
def main():
    ...
    ap.add_argument("--levels", type=str, default="1,2,3",
                    help="Comma-separated level numbers to build (default: 1,2,3).")
    ...
    stages = {lvl: Path(f"/tmp/reharm_hymnal_L{lvl}") for lvl in levels}
