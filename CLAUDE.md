...
- **`harp_mapper.py`** — given a roman numeral + key + (optional) melody note, score the 118-fraction pool (paths + reserve) and return the best LH/RH fraction. The pool is **strictly diatonic by design** — no tritone subs, no ♭II7, no altered dominants, no harmonic-minor V. For minor-key hymns, RNs are translated to the relative major because the pool entries are Ionian-labeled. The mapper is **cycle-aware**: it detects 2nds/3rds/4ths cycle transitions between adjacent RNs and picks `paths` entries with CW/CCW direction inferred from melodic contour (ascending → CW, descending → CCW). Harmonic-minor V is handled by one of four substitution strategies (`bVII_backdoor` / `III_deceptive` / `pedal_i` / `modal_v`) and the chosen strategy is recorded on the assignment as `harmonic_substitution`, with the original RN preserved as `requested_rn`.
...
- **Harmonic-minor V can't live in the 118-fraction pool** — it requires a chromatic leading tone. The mapper falls back to one of four substitution strategies; this is by design.
