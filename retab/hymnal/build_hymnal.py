#!/usr/bin/env python3
"""Bulk-build Retab Hymnal at multiple levels, rendered through the abcjsharp FORK
(jazz/fork_render.js) so it matches the rest of the harp apps: harp grand-staff stems
(V:1 up / V:2 down) and middle-C centered between the staves -- NOT abcm2ps. Writes:

  - tablet_app/assets/retab/hymns/L<level>/<slug>.svg
...
from retab_hymnal import RETAB_LEVELS, build_abc            # noqa: E402
...
ASSETS = Path("/home/james.clements/projects/HarpHymnal/tablet_app/"
              "app/src/main/assets/retab")
INDEX_JS = ASSETS / "retab_hymns.js"
STAFFWIDTH = 1000
...
def main():
    ...
    for num, jp in numbered:
        ...
        for lvl in levels:
            try:
                leveled = RETAB_LEVELS[lvl](json.loads(raw))
                abc = strip_abcm2ps(build_abc(leveled, x_num=1, num_prefix=num, level=lvl))
            except Exception as e:
    ...
    INDEX_JS.write_text("window.RETAB_HYMNS = "
