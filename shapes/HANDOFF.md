...
1. **Move any now-unblocked items** out of *Outstanding blockers*.
1. If a role boundary just shifted — note it under *Role overrides this week*.
...
**Key reframe (do not lose this):** the system encodes *what is committed
to in the air before the hand touches the strings*. Rhythm, articulation,
strike order after contact, timing, and dynamics are deliberately outside
scope. Most of what looks like “playing notation” (operators, hand
markers, space-separated shape sequences) is actually about sequences of
*pre-contact configurations*, not sequences of *played notes*. There’s a
project-memory entry under `memory/project_encoding_scope.md` — read that
first if you didn’t participate in today’s session.

**What changed in the file system today (uncommitted):**

- `README.md` — full rewrite. Three octave-prefix forms: absolute
  (`1x`, `2L`, `7R` — explicit harp octave 1–7 plus letter slot for
  hand), relative (`+1x`, `-2L` — sign required, prior-octave plus/minus
  N), and absent (sticky inheritance from prior shape; drill context
  may leave it unstated entirely). Disambiguator is the first character
  class. Subscripts now mean **assembly order** (which finger plants
  1st–8th in the assembly sequence; ₁–₄ for one-hand shapes, ₁–₈ global
  for two-hand), not finger labels — the previous README had this
  wrong. Hand assignment lives in the **letter slot** of the octave
  prefix (`x` = either, `L` = left, `R` = right): `1L^6` is a left-hand
  shape, `1R^1333` is right-hand. Two-hand shapes (≥ 4 intervals) use
  a comma inside one shape to split LH and RH intervals
  (`1x^1333,3333`). Shapes within a row are space-separated. Header
  block: `Title:` (optional) + `Key:` (required), separated from rows
  by `---`. No `Meter:` — rhythm is out of scope.
...
- `DRILLS.md` — full replacement. Old chord-pool letter notation
  (`cegDFA` etc.) is gone. Drills under the new grammar, including
  the canonical thirds chain (`1x^1333$~`), modal cycle, two-hand
  bass+chord progressions (`1L^1 3R^1333 …`), and a top-down assembly
  study using subscripts (`1x^1₄3₃3₂3₁`).
...
1. `README.md` (the new spec)
1. `DRILLS.md` (sample sequences under the new grammar)
1. `HANDOUT.md` (the pattern catalog)
1. `VERIFY.md` (calibration drills, awaiting user results)
...
- Multi-row composition is sequential phrases (decided); true
  parallel-row simultaneity is deferred. The `$` and `&` operators are
  in the spec for shape-sequence expansion (back-to-back repeat;
  octave-aligned re-stamp).
...
1. **Docs feature shipped (`4899801`)**: new **Docs** tile on the home
...
1. **There was a false-start push `2f9f3c2`** — first attempt landed
   ...
   stretch, that’s why.
1. **Tablet already has `4899801` installed** from the lab today.
   Check via:
...
1. **The orphan `jazz/` tree** will probably be deleted eventually.
...
- **2026-04-24 lab** — `4899801` `tablet_app: Docs tile + markdown viewer (re-land from dead jazz/)` — landed `docs.html` + vendored
...
- **2026-04-24 lab** — `2f9f3c2` `jazzhymnal: Docs tile + in-app markdown viewer` — **SUPERSEDED by `4899801`.** Landed under `jazz/`, which
  was already dead after `5f2b46b`; nothing from this push reached the
  APK. Kept for history only.
- **2026-04-24 lab** — `5f2b46b` `Consolidate retab + reharm into HarpHymnal, drop OBE subprojects` — major restructure. Deleted
...
- **2026-04-21 lab** — `17ac251` `HANDOFF.md: log 2026-04-21 evening lab session (9d7136f back to d011ce8)` — pure docs entry.
- **2026-04-21 lab** — `9d7136f` `survey: Jazz Version reference track alongside Full Baseline` — generated 40 Amazing Grace variations via
  ...
  obsolete.)
- **2026-04-21 lab** — `e26fcab` `survey: louder default + wider slider range + sqrt-velocity curve` — tablet reported “can barely hear”.
...
- **2026-04-21 lab** — `8546816` `survey: volume slider + note-off honored in MIDI playback` — added `🔈 [slider] %` widget driving a
  master `GainNode`. Rewrote the `MidiPlayer` event callback so
  Note-on/Note-off are honored with a 1.2s release tail.
- **2026-04-21 lab** — `d011ce8` `survey: bundle MIDI playback libs offline (tablet has no wifi)` — vendored `midi-player-js@2.0.16` +
  ...
  tablet).
- **2026-04-21 lab** — `105b1b1` `HANDOFF.md: direct brief section for home-laptop Claude`.
- **2026-04-21 lab** — `c55d000` `tactics: populate music-theory notes for all 79 tactics` — filled the `note` field on all 73 previously-
...
- **2026-04-21 lab** — `97861fd` `tactics survey: dimension-level help buttons + theory notes`.
- **2026-04-21 lab** — `d4dfb63` `reharm_solo: land Amazing Grace survey-audio fixtures` — committed 158 tactic/original MIDI pairs. (Deleted in
  `5f2b46b`.)
- **2026-04-21 home** — `3e1b638` `jazzhymnal: new WebView-wrapper tablet app` — scaffolded `jazzhymnal/`. (Project deleted in `5f2b46b`.)
- **2026-04-21 home** — (same push) Help-icon modal on tactic rows in
  `test_amazing_grace.html`.
- **2026-04-21 lab** — `b06dde6` `Audition rig → snippet A/B + 0-5 scoring; TREFOIL corrections; lab handoff`.
- **2026-04-20 lab** — `74c3472` `Reharm Tactics: 8-phase pipeline from tactic pool to tablet UI`.
